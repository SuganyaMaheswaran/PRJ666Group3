...
async function materialize(supabase, userId, template, items, arrivalDate) {
  ...
  while (level.length > 0) {
    ...
    created.forEach((node, i) => itemIdToNodeId.set(level[i].item_id, node.task_node_id));
  ...
  return { generated, templateId: template.template_id, templateVersion: template.version };
}
...
async function generateTasksForUser(supabase, userId, immigrationStatus, arrivalDate) {
  ...
  // Idempotency guard: templates are immutable once assigned, and a user
  // should never receive the same onboarding set twice. Scoped to
  // non-compliance TEMPLATE nodes so this doesn't collide with (or get
  // tripped up by) the separate compliance generation below.
  const existing = await taskNodeRepo.findAllByUser(supabase, userId);
  if (existing.some((n) => n.source === "TEMPLATE" && (n.task_category ?? "GENERAL") !== "COMPLIANCE")) {
  ...
  return materialize(supabase, userId, template, items, arrivalDate);
}
...
async function generateComplianceTasksForUser(supabase, userId, immigrationStatus) {
  ...
  const existing = await taskNodeRepo.findAllByUser(supabase, userId);
  if (existing.some((n) => n.source === "TEMPLATE" && n.task_category === "COMPLIANCE")) {
    return { generated: 0, reason: "already_generated" };
  }
  ...
  return materialize(supabase, userId, template, items, null);
}
