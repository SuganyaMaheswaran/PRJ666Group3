...
describe("generateTasksForUser", () => {
  ...
  test("is idempotent — skips generation if the user already has template-sourced tasks", async () => {
    taskNodeRepo.findAllByUser.mockResolvedValue([{ task_node_id: 1, source: "TEMPLATE" }]);
    const result = await service.generateTasksForUser({}, "user-1", "International Student", "2026-01-01");
    expect(result).toEqual({ generated: 0, reason: "already_generated" });
    expect(templateRepo.findActiveByCategory).not.toHaveBeenCalled();
  });
  ...
  test("materializes a nested template breadth-first, wiring parent_id from the previous level", async () => {
    ...
    templateRepo.findItemsByTemplate.mockResolvedValue([
      { item_id: 100, parent_item_id: null, title: "Prepare College Application", description: null, default_due_offset_days: 30, sort_order: 1 },
      { item_id: 101, parent_item_id: 100, title: "Collect transcripts", description: null, default_due_offset_days: 10, sort_order: 1 },
      { item_id: 102, parent_item_id: 100, title: "Upload documents", description: null, default_due_offset_days: 20, sort_order: 2 },
      { item_id: 103, parent_item_id: 101, title: "Request from registrar", description: null, default_due_offset_days: null, sort_order: 1 },
    ]);
    ...
    expect(result).toEqual({ generated: 4, templateId: 10, templateVersion: 1 });
...
describe("generateComplianceTasksForUser", () => {
  ...
  test("is idempotent — skips generation if the user already has compliance template tasks, but is unaffected by onboarding tasks", async () => {
    taskNodeRepo.findAllByUser.mockResolvedValue([{ task_node_id: 1, source: "TEMPLATE", task_category: "GENERAL" }]);
    templateRepo.findActiveByCategory.mockResolvedValue(null);
    const result = await service.generateComplianceTasksForUser({}, "user-1", "International Student");
    expect(result.reason).not.toBe("already_generated");
  });

  test("skips generation if compliance tasks already exist", async () => {
    taskNodeRepo.findAllByUser.mockResolvedValue([{ task_node_id: 1, source: "TEMPLATE", task_category: "COMPLIANCE" }]);
    const result = await service.generateComplianceTasksForUser({}, "user-1", "International Student");
    expect(result).toEqual({ generated: 0, reason: "already_generated" });
    expect(templateRepo.findActiveByCategory).not.toHaveBeenCalled();
  });
  ...
  test("looks up the COMPLIANCE template kind, separate from the onboarding one", async () => {
    ...
    templateRepo.findItemsByTemplate.mockResolvedValue([
      { item_id: 200, parent_item_id: null, title: "Study Permit Conditions", description: null, task_category: "COMPLIANCE", priority: "NORMAL", default_due_offset_days: null, sort_order: 1 },
      { item_id: 201, parent_item_id: 200, title: "Enroll full-time at your DLI", description: "...", task_category: "COMPLIANCE", priority: "HIGH", default_due_offset_days: null, sort_order: 1 },
    ]);
    taskNodeRepo.insertMany
    ...
    expect(result).toEqual({ generated: 2, templateId: 20, templateVersion: 1 });
    expect(taskNodeRepo.insertMany).toHaveBeenNthCalledWith(2, {}, [
